...
class QuizRepository:
    ...
    def get_share_by_quiz(
        self, quiz_id: str, user_id: str
    ) -> dict[str, Any] | None:
        """Return the owner's existing share for a quiz, if any."""
        result = (
            self.supabase.client.table("quiz_shares")
            .select("*")
            .eq("quiz_id", quiz_id)
            .eq("user_id", user_id)
            .maybe_single()
    ...
    def create_share(
        self, quiz_id: str, user_id: str, share_token: str
    ) -> dict[str, Any]:
        """Insert a new share row with the given opaque token."""
        result = (
            self.supabase.client.table("quiz_shares")
            .insert({
                "quiz_id": quiz_id,
    ...
    def get_share(self, share_token: str) -> dict[str, Any] | None:
        """Load an active share by its public token (no user scoping)."""
        result = (
            self.supabase.client.table("quiz_shares")
            .select("*")
            .eq("share_token", share_token)
            .eq("is_active", True)  # noqa: FBT003
            .maybe_single()
            .execute()
        )
        if not result or not result.data:
    ...
    def increment_share_open(self, share_id: str) -> None:
        """Bump the open counter + last-opened timestamp for a share."""
        self.supabase.client.rpc(
            "increment_share_open", {"p_share_id": share_id}
        ).execute()

    def increment_share_attempt(self, share_id: str) -> None:
        """Bump the attempt counter for a share."""
        self.supabase.client.rpc(
            "increment_share_attempt", {"p_share_id": share_id}
        ).execute()

    def insert_share_attempt(
        self,
        share_id: str,
        quiz_id: str,
        evaluation: dict[str, Any],
        ...
        (
            self.supabase.client.table("quiz_share_attempts")
            .insert({
                "share_id": share_id,
                "quiz_id": quiz_id,
